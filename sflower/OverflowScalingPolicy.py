    ...
    for x in range(0, pending_jobs_len):
        ...
        try:
            if move_cursor < number_of_pods_to_move:
                ...
                logging.info("successfully moved job: move_cursor: " + str(move_cursor) + ", moving job to cluster: " + job.metadata.name)
            else:
                logging.info("move_cursor: " + str(move_cursor) + ", job staying queued: " + job.metadata.name)
            move_cursor = move_cursor + 1
        except:
            logging.info("Unexpected error moving job, cursor: " + str(move_cursor) + ",job: " + job.metadata.name)
